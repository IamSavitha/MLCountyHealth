    ...
    \begin{enumerate}
        \item \textbf{Comprehensive multi-algorithm analysis:} Application of 10+ machine learning algorithms across regression, classification, and clustering tasks to the same dataset, enabling robust comparison of modeling approaches.
        ...
        \item \textbf{Interactive dashboard:} Streamlit-based visualization platform for stakeholder engagement and exploratory analysis.
    \end{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Lasso Regression (L1):} Feature selection through coefficient shrinkage with same $\alpha$ range
    \end{itemize}
    ...
    \textbf{Algorithms:}
    \begin{itemize}
        \item \textbf{Logistic Regression:} Linear decision boundary, max iterations = 1000
        \item \textbf{K-Nearest Neighbors:} k=5, Euclidean distance
        \item \textbf{Gaussian Naive Bayes:} Probabilistic classifier assuming feature independence
        \item \textbf{Support Vector Machine:} RBF kernel, probability estimates enabled
        \item \textbf{Decision Tree:} Max depth = 5 (prevent overfitting)
        \item \textbf{Random Forest:} 100 estimators, max depth = 5
        \item \textbf{Extra Trees:} 100 estimators, max depth = 5, random split thresholds
    \end{itemize}
    ...
    \textbf{Algorithms:}
    \begin{itemize}
        \item \textbf{K-Means:} Tested k $\in \{2,3,4,5,6,7,8\}$ using elbow method and silhouette scores. Selected k=5 based on interpretability and silhouette score = 0.44.
        \item \textbf{Hierarchical Agglomerative:} Ward linkage, dendrogram analysis, cut at 5 clusters for comparison.
    \end{itemize}
    ...
    \midrule
    \textbf{Mean}  & 0.809 & 0.805 & \textbf{-0.4\%} \\
    \bottomrule
    \end{tabular}
    \end{table}
    ...
    \begin{enumerate}
        ...
        \item Achieving strong predictive performance: 41.7\% $R^2$ for obesity regression, 84.8\% F1 for income inequality classification
        \item Developing an interactive Streamlit dashboard for stakeholder exploration of findings
    \end{enumerate}
    ...
    \textbf{Savitha Vijayarangan:} Conceptualization (Lead), Data Curation (Lead), Formal Analysis (Lead), Investigation (Lead), Methodology (Lead), Software (Lead), Visualization (Lead), Writing - Original Draft (Lead), Writing - Review \& Editing (Lead)

    \textbf{Rishi Patel:} Data Curation (Supporting), Formal Analysis (Supporting), Investigation (Supporting), Software (Supporting)

    \textbf{Kapil Kumar:} Formal Analysis (Supporting), Software (Supporting), Visualization (Supporting)

    \textbf{Jane Heng:} Formal Analysis (Supporting), Methodology (Supporting), Writing - Review \& Editing (Supporting)
