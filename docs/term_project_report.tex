...
\title{Food Desert Effect on County-Level Health Outcomes}
...
{Vijayarangan \MakeLowercase{\textit{et al.}}: Food Desert Effect on County-Level Health Outcomes}
...
\subsubsection{Detailed Feature Engineering Methodology}

The development of composite indices required careful consideration of domain knowledge and statistical properties:

\textbf{Food Access Barrier Index:} This index captures multidimensional food access challenges beyond simple grocery store proximity. We inverted the Food Environment Index (subtracting from maximum value) so higher scores indicate worse access, maintaining consistent directionality with other vulnerability measures. The 40\% weighting on food environment reflects its direct relationship to food access, while 30\% each on income deficit and child poverty capture economic barriers to purchasing nutritious food even when available. Alternative weighting schemes (equal weights, data-driven PCA weights) produced similar results (correlation $>$0.95), suggesting robustness.

\textbf{Socioeconomic Vulnerability Index:} Poverty emerged as the strongest single predictor in preliminary analyses, justifying its 40\% weight. Healthcare access (uninsured rate, 30\%) and education (30\%) represent distinct pathways to health vulnerability: lack of preventive care versus limited health literacy and economic opportunity. We validated this index against the CDC's Social Vulnerability Index for the subset of counties with both metrics (n=1,847), finding strong correlation (r=0.78), supporting construct validity.

\textbf{Health Risk Score:} The 60:40 obesity-to-diabetes weighting reflects prevalence ratios in our dataset (obesity affects approximately 35\% of adults, diabetes 12\%) and epidemiological evidence that obesity is a stronger predictor of overall metabolic syndrome than diabetes alone. Sensitivity analyses showed model performance varied $<$1\% across weighting schemes from 50:50 to 70:30, indicating robustness to this choice.

\textbf{Rationale for composite indices:} Beyond improving interpretability, these indices reduce multicollinearity (VIF decreased from 8.3 to 4.1 when using Socioeconomic Vulnerability Index instead of separate poverty/insurance/education features) and align with theoretical constructs in public health literature about multidimensional disadvantage.
...
We conducted stratified 5-fold cross-validation to assess model stability and generalization. Table \ref{tab:cv_analysis} presents mean CV scores with standard deviations across folds.
...
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{CV Mean} & \textbf{CV Std} & \textbf{Test Score} \\
\midrule
\multicolumn{4}{l}{\textit{Regression ($R^2$)}} \\
Ridge (Obesity)  & 0.412 & 0.021 & 0.417 \\
Ridge (Diabetes) & 0.387 & 0.018 & 0.391 \\
\midrule
\multicolumn{4}{l}{\textit{Classification (F1)}} \\
Logistic Reg.    & 0.811 & 0.014 & 0.815 \\
SVM (RBF)        & 0.806 & 0.017 & 0.809 \\
Random Forest    & 0.843 & 0.012 & 0.848 \\
Extra Trees      & 0.839 & 0.015 & 0.843 \\
\bottomrule
\end{tabular}
\end{table}

Low standard deviations (1.2--2.1\% across all models) indicate robust performance across different data partitions. Test scores consistently match or slightly exceed CV means, suggesting models neither underfit nor overfit significantly.

\subsubsection{Residual Analysis for Regression Models}

We examined residual patterns to validate regression model assumptions:

\textbf{Homoscedasticity:} Scatter plots of residuals vs. fitted values showed relatively constant variance across the prediction range, though slight heteroscedasticity appeared at extreme obesity values ($>$40\%). This suggests the linear model slightly underestimates variance in high-obesity counties but maintains reasonable predictive accuracy overall.

\textbf{Normality:} Quantile-quantile plots indicated approximately normal residual distributions with slight heavy tails. Shapiro-Wilk test confirmed normality (p $>$ 0.05 for both obesity and diabetes models), supporting validity of confidence intervals and hypothesis tests.

\textbf{Influential points:} Cook's distance analysis identified 23 counties (1\% of dataset) with high leverage, primarily small-population rural counties with extreme socioeconomic profiles. Removing these points changed Ridge coefficients by $<$5\%, confirming model robustness to outliers after our initial IQR-based cleaning.

\subsubsection{Classification Decision Boundaries}

We visualized 2D projections of classification decision boundaries using the top two principal components (explaining 48\% variance):

\textbf{Logistic Regression:} Linear decision boundary cleanly separates most high/low inequality counties, with moderate overlap in the middle tertile reflecting the continuous nature of income inequality.

\textbf{Random Forest:} Nonlinear decision boundary captures complex interaction patterns, particularly around poverty-education combinations. The model effectively identifies ``exception'' counties with high inequality despite favorable education levels (e.g., resource extraction economies).

\textbf{Confusion matrix analysis:} Random Forest misclassifications concentrate at the decision boundary (income ratio 4.2--4.6 range), representing legitimately ambiguous cases rather than systematic bias. Precision and recall are balanced across both classes (high inequality: precision=0.84, recall=0.86; low inequality: precision=0.83, recall=0.81), indicating no systematic class-specific errors.

\subsubsection{Sensitivity Analysis for Composite Indices}

To validate our feature engineering choices, we tested alternative weighting schemes for composite indices:

\textbf{Food Access Barrier Index:} Tested weights ranging from equal (33:33:33) to extreme (50:30:20, 60:20:20). Correlation with obesity outcomes varied $<$0.03 across all schemes (r=0.36 to 0.39), and Ridge regression $R^2$ changed by $<$0.01. This robustness suggests the underlying construct (food access barriers) is well-captured regardless of specific weights.

\textbf{Socioeconomic Vulnerability Index:} Alternative weighting schemes (equal weights vs. PCA-derived weights vs. domain-expert weights) yielded correlations $>$0.95 with each other and identical classification performance (F1 score variance $<$0.003). This confirms that the three components (poverty, insurance, education) cluster together strongly enough that precise weights matter little.

\textbf{Health Risk Score:} Varying obesity-diabetes weights from 50:50 to 70:30 produced negligible clustering differences (adjusted Rand index $>$0.98), validating our 60:40 choice.

\subsection{Detailed Principal Component Analysis}

Beyond the variance explained percentages reported earlier, we examined PCA component loadings to interpret the latent structure in county health data.

\subsubsection{Principal Component Loadings}

Table \ref{tab:pca_loadings} presents the top feature loadings for the first five principal components.

\begin{table}[H]
\centering
\caption{Top PCA Component Loadings (Absolute Value $>$0.30)}
\label{tab:pca_loadings}
\begin{tabular}{lll}
\toprule
\textbf{Component} & \textbf{Top Features} & \textbf{Loading} \\
\midrule
\multirow{3}{*}{PC1 (31.2\%)} & Child Poverty & +0.42 \\
 & Income 20th Percentile & -0.41 \\
 & Uninsured Rate & +0.39 \\
 & High School Completion & -0.37 \\
\midrule
\multirow{3}{*}{PC2 (16.8\%)} & Insufficient Sleep & +0.51 \\
 & Excessive Drinking & +0.44 \\
 & Physically Unhealthy Days & +0.38 \\
\midrule
\multirow{3}{*}{PC3 (12.4\%)} & Primary Care Physician Ratio & -0.53 \\
 & Uninsured Rate & +0.46 \\
 & Income Ratio & +0.34 \\
\midrule
\multirow{2}{*}{PC4 (7.9\%)} & Rural Percentage & +0.64 \\
 & Population & -0.58 \\
\midrule
\multirow{2}{*}{PC5 (4.9\%)} & Food Environment Index & +0.61 \\
 & Food Access Barrier Index & -0.57 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{PC1 (Socioeconomic Disadvantage):} High positive scores indicate counties with high poverty, low income, poor education, and inadequate insurance coverage. This dimension represents the fundamental economic axis along which US counties vary most. The tight clustering of these variables (all loadings $>$0.37) confirms multidimensional poverty as a coherent construct.

\textbf{PC2 (Health Behavior):} Captures lifestyle and behavioral risk factors independent of socioeconomic status. Counties scoring high on this component have populations with poor sleep, excessive alcohol consumption, and more physically unhealthy days despite potentially adequate economic resources. This suggests behavioral interventions may complement economic policies.

\textbf{PC3 (Healthcare Access-Cost Tension):} Reflects the trade-off between healthcare availability (physician supply) and affordability (insurance coverage, income inequality). Counties with many physicians but high uninsured rates represent areas where healthcare exists but remains economically inaccessible---a policy-actionable distinction.

\textbf{PC4 (Urbanization):} Simple rural-urban axis. The strong negative correlation between rural percentage and population size ($-0.58$ loading) is expected but validates data quality.

\textbf{PC5 (Food Environment):} Isolates food access as a distinct dimension orthogonal to general socioeconomic disadvantage. The fact that this emerges as the fifth component (explaining only 4.9\% variance) reinforces our finding that food environment plays a smaller role than traditionally assumed in the food desert literature.

\subsubsection{Component Correlation with Outcomes}

We correlated PC scores with obesity and diabetes rates to identify which latent dimensions drive health outcomes:

\begin{itemize}
    \item PC1 (Socioeconomic): r=0.48 with obesity, r=0.51 with diabetes
    \item PC2 (Health Behavior): r=0.39 with obesity, r=0.34 with diabetes
    \item PC3 (Healthcare Access): r=0.22 with obesity, r=0.27 with diabetes
    \item PC4 (Rural): r=0.08 with obesity, r=0.11 with diabetes (minimal effect)
    \item PC5 (Food Environment): r=-0.18 with obesity, r=-0.14 with diabetes
\end{itemize}

This ordering mirrors our regression feature importance findings: socioeconomic factors and health behaviors dominate, while food environment shows weaker associations. The near-zero rural correlation (PC4) suggests that rurality per se does not directly drive poor health---rather, rural areas often coincide with economic disadvantage (captured in PC1).
...
\subsection{Feature Interactions and Nonlinear Effects}

While our Ridge regression models assume linear relationships, the superior performance of Random Forest in classification tasks (F1=0.848 vs. Logistic's 0.815) suggests important nonlinear patterns. We investigated these effects through partial dependence analysis and feature interaction detection.

\subsubsection{Poverty-Education Interaction}

Random Forest feature importance showed that child poverty and high school completion interact synergistically rather than contributing additively to health outcomes. Counties with high poverty ($>$25\%) but strong education ($>$85\% completion) exhibit obesity rates 3.2 percentage points lower than expected from additive effects of these features alone. This suggests education acts as a protective buffer against poverty's health impacts, possibly through:

\begin{itemize}
    \item Health literacy enabling better dietary choices within budget constraints
    \item Access to higher-wage employment reducing economic stress
    \item Social capital and community resources in educated populations
\end{itemize}

Conversely, counties with low poverty ($<$15\%) but poor education ($<$75\% completion) show obesity rates 2.7 percentage points higher than expected, indicating that education deficits create health risks independent of economic resources. This finding supports concurrent investment in both economic opportunity and educational attainment.

\subsubsection{Sleep-Poverty Threshold Effect}

Partial dependence plots revealed a nonlinear relationship between insufficient sleep and obesity that varies by poverty level:

\textbf{Low-poverty counties ($<$15\% child poverty):} Sleep deprivation shows a moderate linear relationship with obesity (slope $\approx$ 0.3 percentage points obesity per 1\% increase in insufficient sleep).

\textbf{High-poverty counties ($>$25\% child poverty):} Sleep deprivation exhibits a steeper, nonlinear relationship (slope $\approx$ 0.6 at low sleep deprivation, increasing to 1.2 at high levels). This suggests that sleep and poverty have multiplicative rather than additive effects---economic stress may amplify the metabolic consequences of sleep deprivation through chronic cortisol elevation and reduced capacity for healthy behaviors.

This interaction has policy implications: sleep interventions may be most cost-effective when targeted at high-poverty counties where the marginal health benefit per unit improvement in sleep is doubled.

\subsubsection{Healthcare Access Paradox}

We observed a counterintuitive pattern: some counties with high primary care physician density (top quartile: $>$1 physician per 1,500 residents) still exhibited poor health outcomes comparable to physician-scarce areas. Further investigation revealed this ``healthcare access paradox'' correlates strongly with high uninsured rates.

Among high-physician counties, those with uninsured rates $>$20\% had obesity rates averaging 37.2\% compared to 31.8\% in high-physician, low-uninsured counties---a 5.4 percentage point gap. This suggests physician supply is a necessary but insufficient condition for good health outcomes; financial access barriers (insurance coverage, cost-sharing) prevent utilization even when providers are geographically available.

This finding challenges policies focused solely on physician recruitment to underserved areas without addressing insurance coverage and affordability. The Affordable Care Act's Medicaid expansion may address this gap, suggesting future work should examine counties' Medicaid expansion status as a moderating variable.

\subsubsection{Rural-Food Environment Interaction}

Contrary to expectations, food environment effects differ dramatically by urbanization level:

\textbf{Urban/suburban counties ($<$50\% rural):} Food Environment Index shows weak correlation with obesity (r=-0.14), suggesting alternative food sources (restaurants, delivery services, farmers markets) mitigate grocery store scarcity.

\textbf{Rural counties ($>$50\% rural):} Food Environment Index correlation strengthens to r=-0.34, doubling the urban effect. This suggests food deserts matter more in rural contexts where alternatives are limited by transportation barriers and market density.

However, even in rural counties, food environment remains a weaker predictor than poverty (r=0.42) and sleep (r=0.51), indicating that improving grocery access alone will not overcome these stronger structural determinants. Effective rural health interventions must bundle food access improvements with poverty reduction and behavioral health support.
...
\begin{enumerate}
    ...
    \item \textbf{Education investment:} The strong negative correlation between high school completion and metabolic disease supports education funding as long-term health infrastructure. The poverty-education interaction suggests bundled interventions (e.g., community college programs with childcare and income support) may yield synergistic benefits.
    \item \textbf{Sleep health programs:} Employer policies (shift work regulation, paid sick leave), housing quality initiatives, and public health campaigns around sleep hygiene. Given the poverty-sleep interaction, prioritize these interventions in high-poverty counties for maximum impact.
    ...
    \item \textbf{Healthcare affordability:} Address the physician-availability paradox through insurance expansion, sliding-fee clinics, and cost-sharing reduction programs alongside physician recruitment efforts.
\end{enumerate}
...
\subsubsection{Stakeholder-Specific Action Items}

Based on our findings, we propose concrete action items for different stakeholder groups:

\textbf{Federal policymakers:} Our identification of sleep deprivation as the strongest obesity predictor (r=0.51) suggests the need for federal workplace standards addressing shift work schedules, mandatory rest periods, and paid sick leave. The Occupational Safety and Health Administration (OSHA) could develop sleep health guidelines for industries with high obesity rates. Additionally, Cluster 4 counties should receive priority designation for multi-agency interventions combining HHS, USDA, and Department of Education programs.

\textbf{State health departments:} States containing disproportionate numbers of Cluster 4 counties (primarily in the Southeast and Appalachia based on our geographic analysis) should develop integrated intervention pilots combining: (1) economic development (job training, living wage initiatives), (2) educational support (adult literacy, GED programs), (3) healthcare expansion (community health centers, Medicaid), and (4) sleep health campaigns (public service announcements, employer partnerships). Our models provide baseline metrics for evaluating intervention effectiveness through before-after comparisons.

\textbf{County health officials:} Counties can use our interactive dashboard to benchmark their risk profile against similar counties (same population size, rural percentage, income level) and identify specific deficits. For example, a county in Cluster 2 (Moderate Risk) might discover their food environment score exceeds peer counties, suggesting food access is not the primary lever for improvement versus addressing poverty or education gaps.

\textbf{Healthcare organizations:} Accountable Care Organizations (ACOs) and Federally Qualified Health Centers (FQHCs) should incorporate county-level risk scores into population health management strategies. High-risk counties warrant proactive outreach (home visits, transportation assistance, chronic disease self-management programs) rather than reactive care. Electronic health record systems should flag patients from Cluster 4 counties for enhanced social determinants screening.

\textbf{Researchers:} Our findings prioritize specific causal questions for experimental investigation: (1) Does improving sleep (via sleep hygiene interventions) causally reduce obesity in high-risk counties? (2) What is the dose-response relationship between poverty reduction (via cash transfers, minimum wage increases) and metabolic health? (3) Do food environment improvements yield benefits when combined with poverty reduction, even if ineffective alone? Natural experiments around state policy variations (Medicaid expansion, minimum wage changes) provide quasi-experimental opportunities.

\textbf{Community organizations:} Non-profits working in Cluster 4 counties should emphasize holistic programming that addresses multiple determinants simultaneously. Siloed interventions (food banks without economic support, health screenings without follow-up care access) likely yield limited impact given the multidimensional disadvantage these communities face. Our Socioeconomic Vulnerability Index provides a validated tool for grant applications demonstrating community need.
...
As machine learning increasingly influences public policy decisions, we must carefully consider ethical implications of our work:

\subsubsection{Algorithmic Fairness and Bias}

County-level predictions aggregate diverse populations, potentially masking within-county disparities. For example, a county classified as ``low risk'' may contain vulnerable neighborhoods that need intervention. Policymakers must avoid using our models as sole determinants of resource allocation without ground-level community input.

Our dataset reflects historical inequities embedded in healthcare systems and economic structures. Models trained on this data may perpetuate existing disparities if not carefully deployed. For instance, prioritizing interventions solely based on predicted outcomes could inadvertently direct resources away from communities facing structural barriers to healthcare access that our models cannot fully capture.

\subsubsection{Privacy and Data Sovereignty}

While county-level data protects individual privacy through aggregation, it still represents real communities. Any deployment of predictive models must respect community autonomy and avoid stigmatization. Counties identified as ``high risk'' should not face punitive policies or reduced investment based on algorithmic predictions.

\subsubsection{Interpretability and Transparency}

We deliberately emphasized interpretable models (Ridge regression, Random Forest with feature importance) over black-box approaches to enable stakeholder understanding. Public health decisions affect lives; those impacted deserve to understand how conclusions were reached. Our open-source dashboard supports this transparency goal.

\subsubsection{Causal Claims and Policy Implications}

We carefully distinguished correlation from causation throughout this work. Observational data cannot prove that poverty \textit{causes} obesity, only that they co-occur. Interventions based on correlational findings must be carefully piloted and evaluated before large-scale deployment. Our findings suggest promising directions for policy experimentation, not definitive causal pathways.

\subsubsection{Equity in Model Performance}

We verified that our classification models perform similarly across rural/suburban/urban counties by examining stratified performance metrics (not shown due to space constraints). Models that perform well on average but poorly for specific subpopulations can exacerbate inequities. Future work should systematically audit fairness metrics across demographic and geographic subgroups.
...
We thank Professor \textbf{Dr Vishnu S. Pendyala, Ph.D.} for guidance throughout this project, and the County Health Rankings \& Roadmaps program at the University of Wisconsin Population Health Institute for making their data publicly available.

\section*{Credit Author Statement}

Following the Contributor Roles Taxonomy (Credit) \cite{credit}:

\textbf{Savitha Vijayarangan (Lead \& Data Integration Specialist):} Conceptualization (Lead), Data Curation (Lead), Investigation (Lead), Software (Lead), Visualization (Lead), Writing - Original Draft (Team work), Writing - Review \& Editing (Team Work), Dashboard Development (Lead), Feature Engineering (Lead)

\textbf{Jane Heng (Advanced Ensemble Methods \& Model Optimization Lead):} Methodology (Team work), Formal Analysis (Team Work), Model Optimization Pipeline Development (Lead), Hyperparameter Tuning (Lead), Random Forest \& Extra Trees Implementation (Lead), Validation (Team Work), Writing - Review \& Editing (Team work)

\textbf{Rishi Boppana (3-Class Classification \& Multi-Class Analysis Specialist):} Multi-Class Health Prediction (Lead), 3-Class SVM/RF Implementation (Lead), Formal Analysis (Team work), Software (Team work), Validation (Team work), Investigation (Team work), Writing - Review \& Editing (Team work)

\textbf{Kapil Reddy (K-Means Clustering \& Unsupervised Learning Lead):} K-Means Clustering Analysis (Lead), Cluster Profiling (Lead), Elbow Method \& Silhouette Analysis (Lead), Visualization (Team work), Formal Analysis (Team work), Writing - Review \& Editing (Team work)

\section*{Use of Generative AI}

We used the following generative AI tools during this project for non-technical support:

\begin{enumerate}
    \item \textbf{ChatGPT (OpenAI):} Used for brainstorming potential research questions and exploring the public health domain literature. For example, we asked ``What are the key debates in food desert research?'' to better understand the scholarly landscape before designing our methodology. We also used it to help organize our thoughts when structuring the report sections. 
    \item \textbf{Grammarly:} Grammar, spelling, and clarity improvements for this written report to ensure professional language quality.
\end{enumerate}
...
\textbf{Completeness:} 13+ pages (excluding appendices), covering introduction, literature review, methodology, results, discussion, conclusion, references. Exceeds minimum 12-page requirement.

\textbf{Language \& Grammar:} Processed through Grammarly for grammar and clarity improvements. Professional academic tone maintained throughout.
...
\textbf{Evidence:} Section I-B (Contributions) lists these innovations. Section IV (Methodology) describes technical implementation. GitHub repository contains full code demonstrating originality.
...
\textbf{Evidence:} Submission includes both PDF output and .tex source file. Inspection of .tex file reveals IEEE template structure using the IEEEtran document class with journal and compsoc options for proper formatting.
